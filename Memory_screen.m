...
% lineWidthPix = 4;     % Set the line width (in Pixels) for our fixation cross
...
%Instructions for phase 1 and 2

Screen('TextSize',w, 20);
Screen('TextFont',w,'Helvetica');
Screen('TextStyle', w, 0);

DrawFormattedText(w,'Memorize the following images.\n Click to begin', 'center', 'center');
Screen('Flip',w);
GetClicks(w);
...
idx = randperm(N);
    ...
    for i = 1:1:N % length(img_textures) is equal to 51. here i put 25 for 25 images
      allFilenames(idx(i)).name;
      Screen('DrawTexture',w,img_textures{i},[], dest_rect);
      WaitSecs(1); % Duration the images will be presented for each(secs)
      Screen('Flip',w);
    end
 
 DrawFormattedText(w, 'The end', 'center', 'center');
 Screen('Flip',w);
 WaitSecs(1.000);
